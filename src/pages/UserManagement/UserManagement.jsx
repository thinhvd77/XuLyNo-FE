...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`${styles.statusBadge} ${styles[cssClass]}`}>
      {displayText}
    </span>
  );
};
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          // Force state update with new array reference and ensure we have valid data
          const usersArray = Array.isArray(users) ? users : [];
          setUsers(prevUsers => {
            const newUsers = (Array.isArray(prevUsers) ? prevUsers : []).map((user) =>
              user.employee_code === userId ? { ...user, ...updatedUser } : user
            );
            return newUsers;
          });
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (!response.ok) {
            const errorData = await response.json();
            throw new Error(
              errorData.message || `Không thể ${action} người dùng.`
            );
          }
          ...
          const newUsers = usersArray.map((u) => {
            if (u.employee_code === userId) {
              ...
              // Create updated user object with the response data
              const updatedUser = { 
                ...u, 
                ...responseData, // Merge the entire response since it should be the updated user
                status: responseData.status || newStatus // Use response status if available, fallback to calculated
              };
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const usersArray = Array.isArray(users) ? users : [];
          setUsers(usersArray.filter((u) => u.employee_code !== userId));
